...
__global__ void TourConstruction_AntThread(float *pheromones,
                                           float *distances_processed,
                                           curandState *states, int *tours,
                                           int N, float alpha) {
  ...
  uint16_t *visited = (uint16_t *)(selection_prob + local_size);
  if (idx < N) {
    for (int i = 0; i < N; i++) {
    ...
    for (int num = 1; num < N; num++) {
      for (int j = 0; j < N; j++) {
        ...
        float current_prob = __powf(pheromones[current * N + j], alpha) *
                             distances_processed[current * N + j];
        selection_prob[local_idx * N + j] =
            (visited[local_idx * N + j] > 0)
                ? 0.0
                : current_prob; // only if not visited
      }
...
std::pair<float, std::vector<int>> AntThread(Graph &graph, int num_iterations,
  ...
  while (true) {
    ...
    local_mem_size = threads.x * graph.N * (sizeof(float) + sizeof(uint16_t));
